...
\begin{scope}[scale=0.75, transform shape]
    \begin{abstractclass}[text width = 5cm]{Text}{20,2}
    ...
    \end{abstractclass}
        \umlnote[draw, text width = 8cm] at (12, -4) (text) {Due to Processing limitations, instance variables (which are static, and initialized in \verb|setup()|) are in a static superclass \verb|TextConstants|.};

    \begin{class}[text width = 4cm]{Display}{12,-6}
        \attribute{-width: int}
	...
	\begin{class}[text width = 4cm]{Input}{12, -2}
        \inherit{Text}
        \attribute{+content: String}
    \end{class}
    \umlnote[draw] at (20, -10.5) (input-note) {\verb|content| is public to allow for the key event handler to manipulate its content.};
    ...
    \begin{class}[text width = 3cm]{Channel}{12,2}
        \inherit{Text}
    ...
    \begin{interface}{Command}{-1,4}
        \operation{+getName(): String}
        \operation{+getHelp(): String}
        \operation{+execute(String[] args): void}
    \end{interface}

    \begin{class}[text width = 6cm]{Instance}{3,-1}
    ...
    \umlnote[draw] at (3, -10) (commands-note) {Any instance will have have, at minimum, \verb|/help| and \verb|/quit|.};
\end{scope}
\end{tikzpicture}
\end{center}
\end{landscape}
