...
\begin{scope}[scale=0.8, transform shape]
    ...
    \end{class}
    \umlnote[draw] at (20, 8) (input-note) {\verb|content| is public to allow for the key event handler to manipulate its content.};
    ...
    \umlnote[draw,text width=6cm] at (4, 8) (user-note) {Whether an instance is running in server mode or client mode depends on whether the relevant variables are \verb|null| or not. Behaviors are loaded into commands/screens at \verb|setup()|, and event handler methods are called by the main class due to Processing program structure.};
\end{scope}
\end{tikzpicture}
%\end{landscape}
\newpage
